...
describe('MoviePresenter', () => {
  it('Should add self and characters links', () => {
    ...
    expect(presented._links.characters).toEqual({
      href: `${baseUrl}/api/v1/characters/movie/1`,
    });
  ...
  it('Should wrap collections with pagination metadata and links', () => {
    ...
    expect(collection._links.next).toEqual({
      href: `${baseUrl}/api/v1/movies?limit=5&page=3`,
    });
    expect(collection._links.prev).toEqual({
      href: `${baseUrl}/api/v1/movies?limit=5&page=1`,
    });
  });
});
